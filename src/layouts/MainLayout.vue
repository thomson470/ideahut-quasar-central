  ...
  <q-layout view="hHh lpR fFf" class="background-layout">
          ...
          <template v-for="(menu, index) in menus" :key="index">
            ...
            <q-expansion-item
              ...
              :default-opened="
                ...
                (active_menu.parent?.parent && menu.id === active_menu.parent.parent.id)
              "
            >
              ...
              <div v-for="child in menu.children" :key="child.id" class="drawer-child">
                ...
                >
                  ...
                  <div v-for="grand in child.children" :key="grand.id">
                    ...
                    <q-item v-else v-ripple clickable @click="on_menu_click(grand)">
            ...
            <q-item v-else v-ripple clickable @click="on_menu_click(menu)">
              <q-item-section
                avatar
                style="margin: 0px !important; padding: 0px !important; min-width: 0px !important"
              >
                <q-icon :name="menu.icon" />
              </q-item-section>
              <q-item-section
                :class="'q-pl-md' + (active_menu.id === menu.id ? ' text-weight-bold' : '')"
              >
                {{ menu.title }}
              </q-item-section>
            </q-item>
    ...
    <q-page-container>
      <q-page>
        ...
        <router-view />
        <q-page-scroller position="bottom-right" :scroll-offset="120" :offset="[12, 12]">
          <q-btn round glossy size="sm" icon="keyboard_arrow_up" color="primary" />
        </q-page-scroller>
      </q-page>
    </q-page-container>
  </q-layout>
    ...
    on_menu_click(menu) {
      if (util.isString(menu.link) && '' !== menu.link) {
        if (menu.noPushRoute) {
          ...
          self.$router.push({
            path: menu.link,
            query: Object.fromEntries([...new URLSearchParams(menu.link.split('?')[1])]),
          })
        }
      }
    },
